model SystemZweiterOrdnungKausal  
  ...
  Modelica.Blocks.Sources.Pulse pulse1(amplitude = 0, offset = 0, period = 0.1, startTime = 0.002);
  Modelica.Blocks.Continuous.StateSpace stateSpace1(A = [-R / L, -1 / L; 1 / C, 0], B = [1 / L; 0], C = [0, 1], initType = Modelica.Blocks.Types.Init.InitialState, x(fixed = false), x_start = {5, 0.});
equation
